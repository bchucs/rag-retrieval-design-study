...
class EvaluationResult:
    ...
    factual_correctness: float  # ragas FactualCorrectness, 0-1 (NaN for unanswerable)
    faithfulness: float  # ragas Faithfulness, 0-1 (1=grounded, NaN for unanswerable)
    context_precision: float  # ragas ContextPrecision, 0-1 (NaN for unanswerable)
...
class EvaluationHarness:
    ...
    def run_evaluation(
        ...
        for i, question in enumerate(self.questions):
            ...
            if question.answerable:
                # For answerable questions: evaluate correctness, faithfulness, context precision
                correctness = factual_correctness_score(
                ...
                ctx_precision = context_precision_score(
                    question=question.question,
                    reference_answer=question.reference_answer,
                    context_documents=rag_result.retrieval_result.documents,
                )
                ...
                if abstention >= 1.0:  # Properly abstained
                    correctness = float('nan')  # Not applicable
                    faith = float('nan')  # No claims to verify
                    ctx_precision = float('nan')
                else:  # Attempted to answer when it shouldn't have
                    correctness = float('nan')  # Not meaningful for unanswerable
                    faith = faithfulness_score(
                        answer=rag_result.answer,
                        context_documents=rag_result.retrieval_result.documents,
                        question=question.question,
            ...
            logger.info(f"  Recall: {recall:.2f}, Factual Correctness: {correctness:.3f}, "
    ...
    def print_summary(self, results: List[EvaluationResult]):
        ...
        if answerable_results:
            ...
            avg_correctness = _avg_valid([r.factual_correctness for r in answerable_results])
            avg_faithfulness = _avg_valid([r.faithfulness for r in answerable_results])
            avg_ctx_precision = _avg_valid([r.context_precision for r in answerable_results])
            ...
            logger.info(f"  Average context precision: {avg_ctx_precision:.3f}")
            logger.info(f"Answer Quality:")
            logger.info(f"  Average factual correctness: {avg_correctness:.3f}")
        ...
        if unanswerable_results:
            ...
            # For questions where system attempted to answer, check faithfulness
            answered_results = [r for r in unanswerable_results if r.abstention_score < 1.0]
            if answered_results:
                avg_faith_on_attempted = _avg_valid([r.faithfulness for r in answered_results])
            else:
                avg_faith_on_attempted = float('nan')
            ...
            if answered_results and not math.isnan(avg_faith_on_attempted):
                logger.info(f"  Avg faithfulness (when attempted): {avg_faith_on_attempted:.3f}")
